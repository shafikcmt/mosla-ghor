...
@section('title', 'ভেন্ডর পিকআপ পয়েন্ট')

@section('content')
<div class="flex flex-wrap items-center justify-between gap-3 mb-5">
    <h2 class="text-lg font-bold text-gray-800">ভেন্ডর পিকআপ পয়েন্ট</h2>
    <div class="flex items-center gap-2">
        <form method="GET" action="{{ route('admin.vendor-pickup-points.index') }}" class="flex items-center gap-2">
    ...
    <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
                ...
                <th class="text-left px-4 py-3 font-semibold text-gray-600">পিকআপ</th>
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-3 text-gray-700">{{ $p->vendor?->shop_name ?? '—' }}</td>
                <td class="px-4 py-3 font-medium text-gray-800">{{ $p->pickup_name }}</td>
                <td class="px-4 py-3 text-gray-600">{{ $p->contact_person_name }}<br><span class="text-xs text-gray-400">{{ $p->phone }}</span></td>
                <td class="px-4 py-3 text-gray-600 text-xs">{{ $p->address }}, {{ $p->city }}, {{ $p->district }}</td>
                ...
                <td class="px-4 py-3 text-center">
                    ...
                    @else
                        <form method="POST" action="{{ route('admin.vendor-pickup-points.default', $p) }}" class="inline">
                            @csrf
                ...
                <td class="px-4 py-3 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <a href="{{ route('admin.vendor-pickup-points.edit', $p) }}" class="text-xs text-blue-600 hover:underline">সম্পাদনা</a>
                        <form method="POST" action="{{ route('admin.vendor-pickup-points.destroy', $p) }}"
            ...
            @empty
            <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">কোনো পিকআপ পয়েন্ট নেই।</td></tr>
            @endforelse
        </tbody>
    </table>
</div>

<div class="mt-4">{{ $pickupPoints->links() }}</div>
@endsection
