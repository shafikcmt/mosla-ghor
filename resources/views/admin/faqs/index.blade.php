...
@if($faqs->isEmpty())
    <div class="bg-white rounded shadow px-6 py-10 text-center text-gray-400">
        কোনো FAQ নেই।
        <a href="{{ route('admin.faqs.create') }}" class="text-gray-700 underline ml-1">যোগ করুন</a>
    </div>
@else
<div class="bg-white rounded shadow overflow-hidden">
    <table class="w-full text-sm">
        <thead class="border-b border-gray-200">
            <tr>
                <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">প্রশ্ন</th>
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-5 py-3">
                    <div class="font-medium text-gray-800 leading-snug">{{ Str::limit($faq->question, 80) }}</div>
                    <div class="text-xs text-gray-400 mt-0.5 leading-snug">{{ Str::limit($faq->answer, 100) }}</div>
                </td>
                <td class="px-5 py-3 text-center text-gray-400">{{ $faq->sort_order }}</td>
                <td class="px-5 py-3 text-center">
                    <form action="{{ route('admin.faqs.toggle', $faq) }}" method="POST">
                        @csrf
                        <button type="submit"
                                class="px-2.5 py-1 rounded text-xs font-medium transition-colors
                                       {{ $faq->is_active ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-red-100 text-red-600 hover:bg-red-200' }}">
                ...
                <td class="px-5 py-3 text-right">
                    <div class="flex items-center justify-end gap-3">
                        ...
                        <form action="{{ route('admin.faqs.destroy', $faq) }}" method="POST"
                              onsubmit="return confirm('FAQ মুছে ফেলবেন?')">
                            @csrf @method('DELETE')
