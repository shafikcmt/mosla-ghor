...
<article class="hm-product-tile" data-discovery-product="{{ $product->id }}" data-channel="{{ $previewMode }}">
    <a class="hm-tile-image" href="{{ $previewUrl }}"><img class="product-artwork" src="{{ \App\Support\ProductMedia::url($product->main_image) ?: asset('images/product-placeholder.svg') }}" alt="{{ $product->display_name }}" loading="lazy"></a>
