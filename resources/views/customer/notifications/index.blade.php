...
<div class="flex items-center justify-between mb-5">
    <h2 class="text-lg font-bold text-gray-800">নোটিফিকেশন</h2>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden divide-y divide-gray-50">
    @forelse($notifications as $n)
        ...
        <a href="{{ route('customer.notifications.read', $n->id) }}"
           class="flex gap-3 px-5 py-4 hover:bg-gray-50 transition-colors {{ $n->read_at ? 'opacity-60' : 'bg-green-50/30' }}">
            ...
            @unless($n->read_at)<span class="text-[10px] text-green-600 font-bold uppercase self-start">নতুন</span>@endunless
        </a>
    @empty
