...
<div class="mb-4 flex items-center gap-3">
    <a href="{{ route('customer.wholesale.enquiry.show', $enquiry->id) }}" class="text-gray-500 hover:text-gray-700 text-sm">← Enquiry</a>
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-4">
    <div class="bg-amber-700 px-5 py-3">
    ...
    <div class="p-5 space-y-3 min-h-[300px] max-h-[500px] overflow-y-auto" id="chat-messages">
        ...
        @php $isOwn = $msg->sender_type === 'customer'; @endphp
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
    <form action="{{ route('customer.wholesale.chat.store', $enquiry->id) }}" method="POST" class="flex gap-3">
        @csrf
        ...
        <button type="submit"
                class="bg-amber-600 hover:bg-amber-700 text-white font-semibold px-5 py-2 rounded-xl text-sm transition-colors self-end flex-shrink-0">
            পাঠান →
        </button>
    </form>
